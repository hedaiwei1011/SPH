            ...
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  :key="saleAttr.id"
                  @click="
                    changActive(saleAttr, spuSaleAttr.spuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      //alert(123)
      //派发action
      //1、发请求--将产品加入到数据库  当前是派发了一个action 也想服务器发请求， 判断加入购物车是成功还是失败了
      try {
        let result = await this.$store.dispatch('addOrUpdateShopCart', {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        })
        //浏览器存储功能 html5中新增 本地存储和会话存储
        //产品信息比较复杂 通过会话存储进行传递（非持久化）
        //不允许是对象
